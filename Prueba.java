...
public class Prueba {
    ...
    public static void Select(String[] path){
        try {
            // Verifica que si el XMLStore que queremos existe
            String folderpath = "C:\\Users\\HP\\Documents\\GitHub\\Proyecto_3\\" + path[3];
            File folder = new File(folderpath);
            if (folder.exists()){
                File[] files = folder.listFiles();
                XMLStore Actual = null;
                for (XMLStore Search : ListStore){
                    if (Search.Name.equals(path[3])){
                        Actual = Search;
                        break;
                    }
                } 
                ...
                for (File file: files){
                    String value = ReadXML(folderpath+"\\"+file.getName(), path[3]);
                    String[] values = value.split("/",0);
                    int Dataid = 0;
                    for (int j=0;j < Actual.MountC; j++){
                        if (path[6].equals(Actual.Columns.get(j))){
                            break;
                        }
                        Dataid++;
                    }
                    if (Dataid==Actual.MountC){
                        Dataid = 0;
                    }
                    System.out.println(values[Dataid]);
                    System.out.println(path[6]);
                    System.out.println(path[7]);
                    if (path[6].equals("=")){
                        if (values[Dataid].equals(path[7])){
                            text = text + ReadXML(folderpath+"\\"+file.getName(), path[3]);
                        }
                    }
                    else if(path[6].equals("<")){
                        if (Integer.parseInt(values[Dataid]) < Integer.parseInt(path[7]) ){
                            text = text + ReadXML(folderpath+"\\"+file.getName(), path[3]);
                        }
                    }
                    else if(path[6].equals(">")){
    ...
    public static void funtionDetection(String function) throws Exception{
        ...
        if (Words[0].equals("TABLA")){
            XMLStore Prueba = new XMLStore(Words[1]);
            for (int i = 2;i <Words.length;i++){
                Prueba.AddColumn(Words[i]);
            }
            ListStore.add(Prueba);
            File directorio = new File("C:\\Users\\HP\\Documents\\GitHub\\Proyecto_3\\" + Words[1]);
        ...
        else if (Words[0].equals("SELECT") && Words[1].equals("*") && Words.length==4){
            SelectAll(Words[3]);
        }
    ...
    public static void main(String[] args) throws Exception{
        ...
        while (true) {
            ...
            function = lectura.nextLine();
            if (function.equals("ESC")){
                break;
            }
